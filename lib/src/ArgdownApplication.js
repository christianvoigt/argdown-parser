...
var ArgdownApplication = function () {
  ...
  _createClass(ArgdownApplication, [{
    ...
    key: "runAsync",
    value: function runAsync(request, response) {
      var _this = this;
      ...
      var promise = new Promise().resolve(resp);

      var _loop = function _loop(processorId) {
        var processor = _this.processors[processorId];
        if (!processor) {
          _this.logger.log("verbose", "Processor not found: " + processorId);
          return "continue";
        }

        if (resp.ast && processor.walker) {
          wrappedWalk = function wrappedWalk() {
            processor.walker.walk(request, resp, _this.logger);
            return resp;
          };
        ...
        var _loop2 = function _loop2(plugin) {
          _this.logger.log("verbose", "Running plugin: " + plugin.name);
          if (_.isFunction(plugin.runAsync)) {
            promise = promise.then(plugin.runAsync(request, resp, _this.logger));
          } else if (_.isFunction(plugin.run)) {
            wrappedRun = function wrappedRun(resp) {
              plugin.run(request, resp, _this.logger);
              return resp;
            };
    ...
    value: function run(request, response) {
      ...
      try {
        for (var _iterator7 = processorsToRun[Symbol.iterator](), _step7; !(_iteratorNormalCompletion7 = (_step7 = _iterator7.next()).done); _iteratorNormalCompletion7 = true) {
          var processorId = _step7.value;

          var _processor = this.processors[processorId];
          if (!_processor) {
          ...
          if (resp.ast && _processor.walker) {
            _processor.walker.walk(resp.ast, resp, this.logger);
          }
          ...
          try {
            for (var _iterator8 = _processor.plugins[Symbol.iterator](), _step8; !(_iteratorNormalCompletion8 = (_step8 = _iterator8.next()).done); _iteratorNormalCompletion8 = true) {
